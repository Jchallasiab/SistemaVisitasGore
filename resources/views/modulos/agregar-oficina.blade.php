        ...
        <div class="p-2 sm:ml-64">
            <div class="border-gray-200 rounded-lg dark:border-gray-700 mt-[4.5rem]">
                <div id='recipients' class="py-8 px-4 mt-6 lg:mt-0 rounded shadow bg-white font-sans">
                    <div>
                        <h2 class="h-16 text-center md:text-3xl text-md text-gray-900 font-sans"><strong>AGREGAR OFICINAS</strong></h2>
                    </div>
                ...
                <form class="" method="POST" action="{{ route('agregar-oficina.store') }}">
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-3 ">
                        ...
                        <div>
                            ...
                            <input
                                class="uppercase appearance-none block w-full bg-gray-200 text-gray-700 border border-black-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                onkeyup="javascript:this.value=this.value.toUpperCase();" placeholder="Ingrese el Piso"
                                id="piso" type="text" value="" name="piso">
                            @error('piso')
                                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="flex flex-col py-0">
                            <button
                                class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 rounded-lg">
                                Agregar
                            </button>
                        </div>
                    </div>
                ...
                <h3 class="py-2 text-center text-2xl text-gray-900 font-extrabold"><strong>TABLA DE REGISTRO</strong></h3>

                <table id="tabla02" class="display table table-striped table-bordered stripe hover font-sans" style="width:100%; padding-top: 1em; padding-bottom: 1em;">
                    ...
                    <tbody>
                        @foreach ($oficinas as $id => $oficina)
                            <tr class="table-data-row">
                                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-black">{{ $id+1 }}</td>
                                ...
                                <td class="px-6 py-16">
                                    <div>
                                        <a href="{{ route('agregar-oficina.edit', $oficina->id) }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                            Editar
                                        </a>
                                        <form action="{{ route('agregar-oficina.destroy', $oficina->id) }}" method="POST" style="display: inline;">
                                            @csrf
                                            @method('delete')
