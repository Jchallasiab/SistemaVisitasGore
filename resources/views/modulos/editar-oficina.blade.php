    ...
    <nav>
        <div class="p-6 sm:ml-64">
            <div class="border-gray-200 rounded-lg dark:border-gray-700 mt-[4.5rem]">
                ...
                <div>
                    <a href="javascript:history.back()" class="py-2 text-center tmd:text-3xl text-xl text-gray-900 font-sans-extrabold">
                    ...
                    <div>
                        <h2 class="py-2 text-center md:text-3xl text-xl text-gray-900 font-sans-extrabold "><strong>EDITAR OFICINA: </strong></h2>
                    </div>
                    ...
                    <form class="" method="POST" action="{{ route('agregar-oficina.update', $oficina->id) }}">
                            ...
                            <div class="relative">
                                <select class="block appearance-none w-full bg-gray-200 border border-black-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                    ...
                                    @foreach ($sedes as $sede)
                                        <option value="{{ $sede->id }}" {{ $sede->id == $oficina->sede_id ? 'selected' : '' }}>
                                            {{ $sede->nombre_sede }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-black-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                onkeyup="javascript:this.value=this.value.toUpperCase();" placeholder="Ingrese el Piso" id="piso"
                                type="text" value="{{ $oficina->piso }}" name="piso">
                            @error('piso')
                                <p class="text-red-500 text-xs italic">{{ $message }}</p>
                            @enderror
                        </div>
